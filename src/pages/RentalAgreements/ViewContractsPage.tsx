    ...
    const data: any[] = [
        {
            ...
            contractID: "4",
            ContractDuration: "2/1/2024 -2/1/2025",
            rentAmount: "₹60,000",
        ...
        <>
            <div className="min-h-screen w-full bg-gray-100 flex justify-center">
                <div className="w-full min-h-screen bg-white rounded-md shadow-md">
                    <h2 className="text-xl text-[#3248d6] font-semibold p-6">Contract Renewal Status</h2>
                        ...
                        <table className="w-full">
                            <thead className='bg-blue-50 shadow-inner '>
                                <tr className="text-left text-slate-600 text-[0.8rem] border-b border-slate-100 tracking-wide">
                                    ...
                                    <th className="p-3 font-normal">RENEWAL STATUS</th>
                                    {/* <th className="p-3 font-normal">VIEW DETAILS</th> */}
                                </tr>
                            </thead>
                            <tbody>
                                {currentItems.map((data, index) => (
                                    <tr key={index} className="border-b border-slate-100 hover:bg-gray-50">
                                        <td className="p-6 font-medium text-slate-600">{data.contractID}</td>
                                        <td className="p-3 text-slate-600 font-light">{data.unitName}</td>
                                        <td className="p-3 text-slate-600 font-light">{data.propertyName}</td>
                                        <td className="p-3 text-slate-600 font-medium">{data.ContractDuration}</td>
                                        <td className="p-3 text-slate-600 font-medium">{data.rentAmount}</td>
                                        <td className="px-3 py-4 text-slate-600 font-normal">
                                            <span className={`${statusColors[data.renewalStatus]} px-3 py-1 text-sm rounded-full`}
                                            >{data.renewalStatus}</span>
                                        </td>
                                        {/* <td className="p-3">
