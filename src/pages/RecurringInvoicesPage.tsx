    ...
    const data: any[] = [
        {
            ...
            "unitName": "Unit 1",
            "invoiceDate": "June 01, 2025",
            "dueDate": "June 10, 2025",
            "totalInvoiceAmount": "18,000",
        ...
        },
        {
            ...
            "dueDate": "May 10, 2025",
            "totalInvoiceAmount": "30,000",
            "invoiceStatus": "Overdue"
        },
        {
            "srNo": "3",
            "invoiceNo": "INV-2025-0017",
            propertyName: "Sunrise Meadows",
            tenant: [
                { name: "Neha Kulkarni", email: "neha.kulkarni@gmail.com" },
                { name: "Priya More", email: "priya.more@gmail.com" }
        ...
        <>
            <div className="min-h-screen w-full bg-gray-100 flex justify-center">
                <div className="w-full min-h-screen bg-white rounded-md shadow-md">
                    <h2 className="text-xl text-[#3248d6] font-semibold p-6">Recurring Invoices</h2>
                        ...
                        <table className="w-full">
                            <thead className='bg-blue-50 shadow-inner '>
                                <tr className="text-left text-slate-600 text-[0.8rem] border-b border-slate-100 tracking-wide">
                                    <th className="ps-6 font-normal">INVOICE NO.</th>
                                    <th className="p-3 font-normal">INVOICE DATE</th>
                                    <th className="p-3 font-normal">DUE DATE</th>
                                    <th className="p-3 font-normal">UNIT NAME</th>
                                    <th className="p-3 font-normal">TENANT NAME</th>
                                    <th className="p-3 font-normal">TOTAL INVOICE AMOUNT</th>
                                    <th className="p-3 font-normal">INVOICE STATUS</th>
                            ...
                            <tbody>
                                {currentItems.map((data, index) => (
                                    <tr key={index} className="border-b border-slate-100 hover:bg-gray-50">
                                        <td className="p-6 font-medium text-slate-600">{data.invoiceNo}</td>
                                        <td className="p-3 text-slate-600 font-light">{data.invoiceDate}</td>
                                        <td className="p-3 text-slate-600 font-light">{data.dueDate}</td>
                                        <td className="p-3 text-slate-600 font-light">{data.unitName}<div className='text-sm font-light'>{data.propertyName}</div></td>
                                        ...
                                        <td className="p-3 text-slate-600 font-medium">₹{data.totalInvoiceAmount}</td>
                                        <td className="px-3 py-4 text-slate-600 font-normal">
                                            <span className={`${statusColors[data.invoiceStatus]} px-3 py-1 text-sm rounded-full`}
                                            >{data.invoiceStatus}</span>
                                        </td>
                                        <td className="p-3">
                                            {/* View (Eye) Button */}
