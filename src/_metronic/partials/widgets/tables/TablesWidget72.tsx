...
import { AddReference } from "../../modals/create-app-stepper/AddReference";
import { CreateEditReference } from "../../modals/create-app-stepper/CreateEditReference";
...
const TablesWidget72 = () => {
  ...
  const handleModalEdit = (id: number, source: string, description: string) => {
    setReferenceId(id);
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${DOMAIN}/api/school/delete-reference/${referenceId}`,
        {
          method: "DELETE",
        }
      );
      ...
      toast.success("Reference deleted successfully.", { autoClose: 3000 });
      handleCloseDeleteModal();
    } catch (error) {
      console.error("Error deleting Reference:", error);
      toast.error("Failed to delete Reference!", { autoClose: 3000 });
    }
  };
            ...
            {filteredData.map((item, index) => (
              ...
              >
                ...
                >
                  <div
                    onClick={() =>
                      handleModalEdit(item.id, item.reference, item.description)
                    }
                    style={{
                      display: "flex",
                      alignItems: "center",
