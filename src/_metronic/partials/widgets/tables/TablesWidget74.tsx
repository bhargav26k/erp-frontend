...
const TablesWidget74 = () => {
  ...
  const fetchReportData = async (customFilters = null) => {
    try {
      ...
      // Handle class parameters - send as array
      if (filters.installment?.length > 0) {
        ...
        // If only "all_classes" is selected or array is empty after filtering, don't add the parameter
      }

      const response = await fetch(
        `${DOMAIN}/api/school/get-fee-outstanding-report?${params.toString()}`
      );
      const result = await response.json();
      console.log(result);
      if (result.message === "No outstanding fees found") {
        ...
        setData(result.data || []); // Updated to use result.data with fallback
      }
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    // Step 1: Format the data for the table
    const formattedData = data.map((item, index) => ({
      "Sr.": index + 1,
      "Gen Reg No": item.gen_reg_no || "",
      "Name of Student": item.student_name || "",
      Class: item.class_name || "",
      "Div.": item.division || "",
      "Roll No": item.roll_no || "",
      "Installment name.": item.invoice_name || 0, // 👈 new field from backend
      "Fee to be collected (2025-2026)": item.total_amount || 0,
      "Balance due Rs.": item.balance_due || 0,
      Remarks: item.remarks || "",
    }));

    // Step 2: Create worksheet from JSON (this gives headers + rows automatically)
    const worksheet = XLSX.utils.json_to_sheet(formattedData, { origin: "A6" });

    // Step 3: Add school header info manually
    XLSX.utils.sheet_add_aoa(
      ...
      [
        ...
        [
          `Selected Filters = ${
            selectedInstallment?.map((item) => item.label).join(", ") || ""
          }`,
          "",
          "",
          "",
    ...
    // Apply style to row 6 (headers)
    const tableHeaders = Object.keys(formattedData[0] || {});
    tableHeaders.forEach((header, index) => {
      const cellRef = XLSX.utils.encode_cell({ r: 6, c: index }); // Row 6
    ...
    worksheet["!cols"] = [
      ...
      { width: 8 }, // Div.
      { width: 10 }, // Roll No
      { width: 20 }, // Fee to be collected
      { width: 18 }, // Fee Collected
      { width: 18 }, // Balance Due
      { width: 15 }, // Remarks
    ];

    // Step 7: Wrap inside workbook
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Fee Outstanding Report");

    const fileName = `Fee_Outstanding_Report_${new Date()
      .toISOString()
      .slice(0, 10)}.xlsx`;
