...
const TablesWidget79 = () => {
  ...
  useEffect(() => {
    const fetchCirculars = async () => {
      ...
      try {
        ...
        setFilteredData(data); // Update the filteredData state with the fetched data

        // Process and sort class names for each item in the fetched data
        const sortedData = data.map((item) => {
          const sortedClassNames = item.class_names
            .split(",") // Split the string into an array
            .map((className) => className.trim()) // Trim any extra spaces
            .sort((a, b) => {
              const classNumberA = parseInt(a.split(" ")[1]);
              const classNumberB = parseInt(b.split(" ")[1]);
              return classNumberA - classNumberB; // Compare based on the class number
            })
            .join(", "); // Join the sorted array back into a string

          return {
            ...item,
            class_names: sortedClassNames, // Update the item with sorted class names
          };
        });

        setFilteredData(sortedData); // Update state with sorted class names
      } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `${DOMAIN}/api/student/delete-circular/${school_id}/${session_id}/${circularId}`,
        {
          method: "DELETE",
        }
      );
      if (!response.ok) {
        throw new Error("Failed to delete the Circular");
      }
      setRefresh(true);
      toast.success("Circular deleted successfully.", { autoClose: 3000 });
      handleCloseDeleteModal();
    } catch (error) {
      console.error("Error deleting Circular:", error);
      toast.error("Failed to delete Circular!", { autoClose: 3000 });
    }
  };
  ...
  return (
    ...
    >
                ...
                <td style={{ textAlign: "center", padding: "12px 20px" }}>
                  <span
                    style={{
                      ...
                      borderRadius: "20px",
                      backgroundColor:
                        item.visibility === "public" ? "#E7F8F0" : "#FFF2E5",
                      color:
                        item.visibility === "public" ? "#0F9D58" : "#FF9800",
                      fontWeight: "600",
                      fontSize: "13px",
      ...
      <DeleteConfirmationModal
        ...
        description={`Are you sure you want to delete this Circular ?  \n This action cannot be undone.`}
