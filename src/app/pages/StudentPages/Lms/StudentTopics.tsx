...
const StudenTopicsPage: FC = () => {
  ...
  const class_section_subject_lesson_id = query.get("class_section_subject_lesson_id");
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      if (!school_id || !classId || !section_id || !lesson_id) return;

      try {
        const response = await fetch(
          `${DOMAIN}/api/student/get-lesson-uploads/${school_id}/${classId}/${section_id}/${subject_id}/${lesson_id}`
        );
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        const response = await fetch(
          `${DOMAIN}/api/student/get-alltopics/${class_section_subject_lesson_id}/${school_id}`
        );
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
    ...
    fetchTopics();
  }, [class_section_subject_lesson_id, school_id]);


  const handleCheckTopics = (topicId : string | null) => { 
    const queryParams = new URLSearchParams({
      subject_id:subject_id || '',
      lesson_id:lesson_id || '',
      topic_id : topicId || ''
    }).toString();
      Navigate(`/course-content/topicuploads?${queryParams}`);
            ...
            {getTopics.map((key) => (
                ...
                >
                  <div
                    ...
                    onClick={() => handleCheckTopics(key.topic_id)}
                  >
                    <div
                      className="card-body"
                      style={{
